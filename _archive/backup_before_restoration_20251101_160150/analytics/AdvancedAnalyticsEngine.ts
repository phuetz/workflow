...
export class AdvancedAnalyticsEngine {
  ...
  trackWorkflowExecution(
    ...
    if (status === 'start') {
      metricsCollector.recordWorkflowStart(workflowId, executionId);
    } else if (status === 'complete') {
      metricsCollector.recordWorkflowComplete(executionId, 'success');
    } else if (status === 'failed') {
      metricsCollector.recordWorkflowComplete(executionId, 'failed', data?.error);
    }
  }
  ...
  trackNodeExecution(
    ...
    if (status === 'start') {
      metricsCollector.recordNodeStart(executionId, nodeId, nodeType);
    } else {
      metricsCollector.recordNodeComplete(
        executionId,
        nodeId,
   ...
   */
  getStatistics(): {
    collector: ReturnType<typeof metricsCollector.getStatistics>;
    warehouse: ReturnType<typeof dataWarehouse.getStatistics>;
  } {
    return {
      collector: metricsCollector.getStatistics(),

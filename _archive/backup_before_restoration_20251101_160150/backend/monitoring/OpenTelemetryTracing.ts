...
export class OpenTelemetryTracing extends EventEmitter {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      // Dynamic import of OpenTelemetry packages (only if installed)
      ...
      const { JaegerExporter } = await import('@opentelemetry/exporter-jaeger').catch(() => ({ JaegerExporter: null }));
      const { OTLPTraceExporter } = await import('@opentelemetry/exporter-trace-otlp-grpc').catch(() => ({ OTLPTraceExporter: null }));

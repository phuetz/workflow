...
export class CollaborationService extends EventEmitter {
  ...
  async detectConflicts(workflowId: string, changes: CollaborationChange[]): Promise<CollaborationConflict[]> {
    ...
    const changesByNode = new Map<string, CollaborationChange[]>();
    for (const change of changes) {
      if (change.data.nodeId) {
        const nodeChanges = changesByNode.get(change.data.nodeId) || [];
        nodeChanges.push(change);
        changesByNode.set(change.data.nodeId, nodeChanges);
      }
    }

    // Check for concurrent edits on same node
    for (const [nodeId, nodeChanges] of changesByNode.entries()) {
      if (nodeChanges.length > 1) {
        // Check if changes are from different users and overlapping in time
        ...
        if (userChanges.size > 1) {
          const conflict: CollaborationConflict = {
            ...
            workflowId,
            changeIds: nodeChanges.map(c => c.id),
            type: 'concurrent_edit',
            description: `Multiple users edited node ${nodeId} simultaneously`,
            users: Array.from(userChanges.keys()).map(userId => ({
              id: userId,
              name: nodeChanges.find(c => c.userId === userId)?.userName || 'Unknown'
            })),
            autoResolvable: this.canAutoResolve(nodeChanges),
            createdAt: new Date()
          };
  ...
  private async handleJoinMessage(client: any, message: any): Promise<void> {
    ...
    const wsServer = getWebSocketServer();
    if (wsServer) {
      wsServer.handleJoinRoom(client, {
        ...message,
        data: { roomId }
      });
    }

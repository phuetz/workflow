...
class AIWorkflowService {
    ...
    try {
      ...
      try {
        replaceablePairs.forEach(pair => {
          suggestions.push({
            type: 'replace',
            nodeIds: pair.oldNodes.map(n => n.id),
            description: `Remplacer ${pair.oldNodes.length} nœuds par un seul nœud "${pair.newNodeType}"`,
            impact: {
              performance: 25,
              reliability: 10,
              cost: -5
  ...
  private calculateConfidence(
    ...
    if (nodeType === 'condition' && context.nodeCount > 5 && !context.hasCondition) {
  ...
  private generateDefaultConfig(nodeType: string, context: unknown): unknown {
      httpRequest: {
        method: 'GET',
  ...
  private findReplaceableNodePairs(nodes: WorkflowNode[]): unknown[] {
    ...
    for (let __i = 0; i < nodes.length - 1; i++) {
  ...
  private wouldCreateLoop(nodeType: string, context: unknown): boolean {
    ...
    return nodeType === 'loop' && context.hasLoop;
  }
  ...
  trainModel(historicalWorkflows: unknown[]) {
    ...
    historicalWorkflows.forEach(workflow => {
      ...
      sequences.forEach(sequence => {
        for (let __i = 0; i < sequence.length - 1; i++) {
    ...
    historicalWorkflows.forEach(workflow => {
      workflow.executionHistory?.forEach((execution: unknown) => {
        Object.entries(execution.nodeTimings || {}).forEach(([nodeId, timing]: [string, unknown]) => {
          ...
          if (baseline) {
            ...
            baseline.avg = newAvg;
            baseline.std = Math.sqrt((baseline.std ** 2 * n + (timing.duration - newAvg) ** 2) / (n + 1));
          }
        });
      });
    });
...
export function useAIWorkflowService() {
  const { _nodes, edges, executionHistory } = useWorkflowStore();

    return aiWorkflowService.predictNextNodes(nodes, edges);
  };

    return aiWorkflowService.optimizeWorkflow(nodes, edges);
  };

    return aiWorkflowService.detectPatterns(executionHistory);
  };

      .filter(h => h.nodeExecutions?.[nodeId])
      .map(h => h.nodeExecutions[nodeId]);

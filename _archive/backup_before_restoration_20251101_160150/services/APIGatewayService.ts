...
export class APIGatewayService extends EventEmitter {
    ...
    return async (req: Request, res: Response, next: NextFunction) => {
      ...
      try {
        ...
        if (route.cache.enabled && req.method === 'GET' && response.status === 200) {
          this.cacheResponse(req, route, response.data);
        }
        ...
        this.sendError(res, 500, 'Internal gateway error', traceData, error);
      }
    };
  }
  ...
  private selectWeighted(upstreams: string[], weights?: number[]): string {
    ...
    for (let __i = 0; i < upstreams.length; i++) {
      weightSum += weights[i];
      if (random <= weightSum) {
        return upstreams[i];
      }
  ...
  private async proxyRequest(req: Request, route: Route, upstream: string): Promise<{
    ...
    try {
      ...
      this.connectionCounts.set(upstream, Math.max(0, currentConnections - 1));
    }
  }
  ...
  private sendError(res: Response, status: number, message: string, traceData: Record<string, unknown>, error?: Error): void {
  ...
  private startHealthChecks(): void {
    for (const [upstream] of this.stats.upstreamStats.entries()) {
  ...
  private startUpstreamHealthCheck(upstream: string): void {
    const healthCheck: HealthCheckConfig = {
      ...
      try {
        ...
        if (!isHealthy) {
          logger.warn(`🚨 Upstream health check failed: ${upstream} (${response.status})`);
        }

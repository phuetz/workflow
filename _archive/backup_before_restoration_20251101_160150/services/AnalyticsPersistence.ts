 ...
 * Handles persisting analytics data to various time-series databases
 */
...
class InfluxDBBackend implements PersistenceBackend {
  ...
  async write(dataPoints: MetricDataPoint[]): Promise<void> {
    try {
        ...
        method: 'POST',
        headers: {
          'Authorization': `Token ${this.config.connection.token}`,
...
export class AnalyticsPersistenceService {
      ...
      switch (backendType) {
        case 'influxdb':
          if (this.config.influxdb) {
            const influxConfig: AnalyticsPersistenceConfig = {
              ...this.config,
        ...
        case 'prometheus':
          if (this.config.prometheus) {
            const promConfig: AnalyticsPersistenceConfig = {
              ...this.config,
        ...
        case 'http':
          if (this.config.http) {
            const httpConfig: AnalyticsPersistenceConfig = {
              ...this.config,
  ...
  private setupFlushInterval(): void {
    const { _intervalManager } = require('../utils/intervalManager');

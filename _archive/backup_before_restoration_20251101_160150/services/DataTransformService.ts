...
export class DataTransformService extends BaseService implements ITransformService {
  ...
  private createHelpers(): TransformHelpers {
    return {
      ...
      groupBy: (arr: unknown[], key: string) => {
  ...
  private initializeBuiltinFunctions() {
    const functions: Array<Omit<DataTransformFunction, 'id'>> = [
...

      {
        ...
        implementation: {
          type: 'builtin',
          handler: (arr: unknown[], condition: unknown) => arr.filter(condition)
        },
...

      {
        ...
        implementation: {
          type: 'builtin',
          handler: (arr: unknown[], transform: unknown) => arr.map(transform)
        },
  ...
  validateExpression(expression: string): ValidationResult {
    ...
    try {
      ...
      for (let __i = 0; i < expression.length; i++) {
        ...
        functionMatches.forEach(match => {
          ...
          if (!func) {
            result.warnings.push({
              line: 1,
              column: expression.indexOf(match) + 1,
      ...
      if (variableMatches) {
        result.usedVariables = [...new Set(variableMatches)];
      }
  ...
  async testExpression(expression: string, input: unknown): Promise<PlaygroundHistoryEntry> {
    ...
    try {
      const context: TransformContext = {
        input,
        variables: {},
        functions: this.createFunctionContext(),
        helpers: this.helpers
      };
  ...
  private calculateSuggestionScore(name: string, partial: string): number {
    ...
    for (let __i = 0; i < lowerName.length && partialIndex < lowerPartial.length; i++) {

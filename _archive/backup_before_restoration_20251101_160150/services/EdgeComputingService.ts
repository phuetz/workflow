...
export class EdgeComputingService {
  ...
  async registerEdgeNode(node: Omit<EdgeNode, 'id' | 'lastSeen' | 'metrics'>): Promise<EdgeNode> {
    const newNode: EdgeNode = {
      ...
      id: this.generateId(),
      lastSeen: new Date(),
      metrics: this.createInitialMetrics(node.id || this.generateId())
    };
  ...
  async getFleetAnalytics(nodeIds: string[], _timeRange: { start: Date; end: Date }): Promise<{
    ...
    topNodes: Array<{
      ...
      w.targetNodes.some(nodeId => nodeIds.includes(nodeId))
    );
  ...
  async discoverDevices(nodeId: string, protocol?: string): Promise<IoTDevice[]> {
    ...
    for (let __i = 0; i < Math.floor(Math.random() * 5) + 1; i++) {
      const device: IoTDevice = {
        id: this.generateId(),
        name: `Discovered Device ${i + 1}`,
        type: deviceTypes[Math.floor(Math.random() * deviceTypes.length)] as string,
        manufacturer: 'Generic',
        model: `Model-${Math.floor(Math.random() * 1000)}`,
        version: '1.0.0',
        protocol: protocol || protocols[Math.floor(Math.random() * protocols.length)] as string,
        edgeNodeId: nodeId,
  ...
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  private async executeAlertAction(action: AlertAction, _reading: SensorReading): Promise<void> {
    switch (action.type) {
      case 'notification':
  ...
  private async executeCommand(device: IoTDevice, actuator: IoTActuator, command: ActuatorCommand, _parameters: unknown): Promise<{
    ...
    return {
      ...
      newState: { ...actuator.currentState, lastCommand: command.name, lastUpdate: new Date() }
    };
  }

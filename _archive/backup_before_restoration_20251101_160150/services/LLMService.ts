...
export class LLMService {
  ...
  private async callLocal(provider: LLMProvider, request: LLMRequest, signal: AbortSignal): Promise<LLMResponse> {
    ...
    return {
      ...
      usage: {
        ...
        cost: 0, // Local models typically don't have cost
        currency: 'USD'
      },
      timestamp: new Date().toISOString(),
      latency: 0,
  ...
  private async checkRateLimits(provider: LLMProvider, request: LLMRequest): Promise<void> {
  ...
  private getCurrentUsage(providerId: string): { requestsPerMinute: number; tokensPerMinute: number } {
    // Implementation to track current usage
    return { requestsPerMinute: 0, tokensPerMinute: 0 };
  }

  private calculateCost(providerId: string, modelId: string, usage: unknown): number {
    if (!model) return 0;
  ...
  destroy(): void {
    ...
    // Also cleanup any active requests
  ...
  private initializeProviders(): void {
    ...
    const openaiProvider: LLMProvider = {
      ...
      config: {
        ...
        timeout: ConfigHelpers.getTimeout('llmRequest'),
        retryAttempts: 3
      },
      capabilities: {
        streaming: true,
    ...
    // Register default providers (they will be inactive until configured)

...
export class MarketplaceService {
  ...
  async installApp(appId: string, appInfo: {
    ...
    try {
      ...
      if (this.repository) {
        try {
          await this.repository.createInstallation(installation);
        } catch (error) {
  ...
  async uninstallApp(appId: string): Promise<boolean> {
    try {
      if (this.repository) {
        try {
          await this.repository.deleteInstallation(appId);
        } catch (error) {
          logger.warn('Database operation failed, falling back to local storage:', error);
  ...
  async installPlugin(pluginId: string, version: string = 'latest'): Promise<boolean> {
    try {
      ...
      const { _securityManager } = await import('../security/SecurityManager');
...
const defaultConfig: MarketplaceConfiguration = {
  ...
  autoUpdate: false,
  allowPremium: true,
  trustedPublishers: ['WorkflowPro Team', 'Official Team'],

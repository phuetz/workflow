...
export class PerformanceOptimizationService extends EventEmitter {
  ...
  private recordPerformanceMetrics(): void {
    ...
    monitoringService.recordMetric('performance.cpu_usage', this.metrics.cpuUsage, {}, '%');
    monitoringService.recordMetric('performance.memory_usage', this.metrics.memoryUsage, {}, '%');
    monitoringService.recordMetric('performance.response_time', this.metrics.responseTime, {}, 'ms');
    monitoringService.recordMetric('performance.throughput', this.metrics.throughput, {}, 'rps');
    monitoringService.recordMetric('performance.error_rate', this.metrics.errorRate, {}, '%');
    monitoringService.recordMetric('performance.active_connections', this.metrics.activeConnections);
  }
  ...
  private calculateErrorRate(): number {
    // Calculate from monitoring service or internal counters
  ...
  private getSlowQueries(): Map<string, any> {
    ...
    for (const [query, stats] of this.queryAnalytics.entries()) {

...
export class RealTimeCollaborationService extends EventEmitter {
  ...
  private handleWebSocketMessage(data: unknown): void {
    const { _type, payload, userId, _sessionId } = data; // eslint-disable-line @typescript-eslint/no-unused-vars
    ...
    if (op1.data.nodeId === op2.data.nodeId) {
      return {
        ...
        data: {
          ...op1.data,
          position: {
            x: op1.data.position.x + 10, // Offset to avoid overlap
  ...
  private transformPropertyUpdateOperations(op1: Operation, op2: Operation): Operation {
    ...
    if (op1.data.nodeId === op2.data.nodeId && op1.data.property === op2.data.property) {
      return {
        ...
        data: {
          ...op1.data,
          value: this.mergePropertyValues(op1.data.value, op2.data.value),
    ...
    if (this.positionsOverlap(op1.data.position, op2.data.position)) {
      return {
        ...
        data: {
          ...op1.data,
          position: {
            x: op1.data.position.x + 50,
            y: op1.data.position.y + 50
          }
        }
      };
    }

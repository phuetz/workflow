...
export class SubWorkflowService extends BaseService implements ISubWorkflowService {
  ...
  async createSubWorkflow(
    ...
    this.logger.info('Creating sub-workflow', { name: data.name });
  ...
  async executeSubWorkflow(
    ...
    this.logger.info('Executing sub-workflow', { id, inputs });
  ...
  private async performExecution(
    ...
    try {
      ...
      const executionPromise = this.workflowExecutor.execute(workflow.nodes, workflow.edges);
      ...
      for (const output of subWorkflow.outputs) {
        ...
        if (node?.data?.[output.mapping.sourceField]) {
          outputs[output.name] = node.data[output.mapping.sourceField];
        }
      }
  ...
  private prepareNodes(
    ...
    const nodes = [inputNode, ...subWorkflow.nodes.map(node => ({
      ...
      data: {
        ...
        ...this.resolveInputReferences(node.data, inputs)
      }
    }))];
  ...
  private async handleExecutionError(
    ...
    switch (errorHandling.strategy) {
      case 'retry':
        if (errorHandling.retryPolicy) {
          ...
          this.logger.info('Retrying sub-workflow execution', { 
      ...
      case 'fallback':
        if (errorHandling.fallbackOutput) {
          execution.outputs = errorHandling.fallbackOutput;
          execution.status = 'success';
          execution.logs.push({
            timestamp: new Date(),
            level: 'warn',
    ...
    if (errorHandling.notifyOnError) {
      // Send notification
      this.logger.error('Sub-workflow execution failed', {
        executionId: execution.id,
  ...
  async runTest(testId: string): Promise<TestResult> {
    ...
    this.logger.info('Running sub-workflow test', { testId, name: test.name });
  ...
  async publishToLibrary(subWorkflowId: string, libraryId: string): Promise<void> {
    ...
    this.logger.info('Published sub-workflow to library', { subWorkflowId, libraryId });
  }
  ...
  async searchLibrary(query: string, filters?: LibraryFilters): Promise<SubWorkflow[]> {
    let results = Array.from(this.subWorkflows.values()).filter(sw => {
      ...
      const isPublished = sw.metadata.isPublic === true;

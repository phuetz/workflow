...
return sourceData.map(item => ({
  ...
  public async install(
    ...
    return this.executeOperation('install', async () => {
  ...
  public async uninstall(installationId: string): Promise<void> {
    return this.executeOperation('uninstall', async () => {
  ...
  public async createTemplate(workflow: unknown): Promise<WorkflowTemplate> {
    return this.executeOperation('createTemplate', async () => {
      ...
      const template: WorkflowTemplate = {
        id: templateId,
        name: workflow.name || 'Custom Template',
  ...
  public async publishTemplate(templateId: string): Promise<void> {
    return this.executeOperation('publishTemplate', async () => {
      const template = this.templates.get(templateId);
      if (!template) {
        throw new Error(`Template ${templateId} not found`);
      }
  ...
  public getMarketplace(filters?: TemplateFilters): TemplateMarketplace {
    ...
    const trending = [...templates]
      .filter(t => {
        ...
        return daysSinceUpdate <= 30 && t.rating >= 4.0;
      })

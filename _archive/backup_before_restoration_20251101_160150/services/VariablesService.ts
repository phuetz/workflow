...
export class VariablesService extends BaseService implements IVariablesService {
    ...
    for (const scope of scopePriority) {
      ...
      if (variable) {
        ...
        if (variable.encrypted) {
          return await this.decryptValue(variable.value);
        }
  ...
  async resolveExpression(expression: string, context: VariableContext): Promise<unknown> {
    ...
    const varMatches = expression.matchAll(/\{\{([a-zA-Z_][a-zA-Z0-9_]*)\}\}/g);
    ...
    if (expression.includes('${') || expression.includes('return')) {
      try {
        ...
        const func = new Function(...Object.keys(varObj), `return ${expression}`);
        return func(...Object.values(varObj));
      } catch (error) {
  ...
  async getAvailableVariables(context: VariableContext): Promise<Variable[]> {
    ...
    for (const variable of variables) {
      ...
      if (this.isVariableAccessible(variable, context)) {
        const value = variable.encrypted ? await this.decryptValue(variable.value) : variable.value;
        available.push({
          name: variable.name,
          value,
          type: variable.type,

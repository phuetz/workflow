...
export class DataPipelineService {
  ...
  async validateData(
    ...
    try {
      ...
      // Sanitize data first
      result.sanitizedData = this.sanitizeData(data, schemaObj);
  ...
  async createPipeline(definition: PipelineDefinition): Promise<void> {
    try {
      ...
      await cacheService.set(`pipeline:${definition.id}`, definition, { ttl: 3600 });
  ...
  private async processStage(
    ...
    try {
      ...
      eventNotificationService.notify('pipeline.stage.complete', {
  ...
  private sanitizeData(data: any, schema: DataSchema): any {
    ...
    if (typeof data === 'object' && data !== null) {
      ...
      for (const [key, value] of Object.entries(data)) {
        ...
        if (!key.startsWith('__') && !key.startsWith('$')) {
          sanitized[key] = this.sanitizeData(value, schema);
        }
      }
  ...
  private async executeCustomTransform(data: any, fn: string): Promise<any> {
    // Safely execute custom function

...
export class ExecutionService {
  ...
  private async executeNode(
    ...
    try {
      this.log(execution, 'debug', `Executing node: ${node.id} (${node.data.type})`);
   ...
   */
  private async evaluateCondition(edge: Edge, context: unknown): Promise<boolean> {
   ...
   */
  private resolveValue(path: string, context: unknown): unknown {
  ...
  private async trackExecutionMetrics(execution: WorkflowExecution): Promise<void> {
    ...
    // Track node metrics
    for (const [nodeId, result] of execution.nodeResults.entries()) {
      analyticsService.trackEvent({

...
export class SSOService {
  ...
  private mapSAMLProfile(profile: Profile): SSOUser {
    ...
    // Helper to get first string value
    const getStringValue = (value: string | string[] | undefined): string | undefined => {
      if (Array.isArray(value)) return value[0];
      return value;
    };

    const emailValue = this.getAttributeValue(profile, emailAttr);
    const email = getStringValue(emailValue) || profile.nameID || '';
    const firstName = getStringValue(this.getAttributeValue(profile, firstNameAttr));
    const lastName = getStringValue(this.getAttributeValue(profile, lastNameAttr));
    const displayNameValue = this.getAttributeValue(profile, displayNameAttr);
    const displayName = getStringValue(displayNameValue) || profile.nameID;

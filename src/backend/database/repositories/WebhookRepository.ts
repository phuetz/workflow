...
export class WebhookRepository {
  ...
  /**
   * Find webhook by URL
   * Note: URL is no longer unique since multiple webhooks can exist per workflow.
  ...
  /**
   * Find webhook by workflow ID and path
   */
  async findByWorkflowAndPath(workflowId: string, path: string): Promise<Webhook | null> {
    try {
      return await prisma.webhook.findUnique({
        where: {
          workflowId_path: { workflowId, path },
        },
      });
    } catch (error) {
      logger.error('Error finding webhook by workflow and path:', error);
      throw error;
    }
  }
  ...
  async getStatistics(webhookId?: string) {
    try {
      ...
      const [totalEvents, processedEvents, failedEvents, recentEvents] = await Promise.all([
        prisma.webhookEvent.count({ where }),
        prisma.webhookEvent.count({ where: { ...where, processed: true, processingError: undefined } }),

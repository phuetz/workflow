    ...
    try {
      return await executeInTransaction(async (tx) => {
        ...
        const workflow = await tx.workflow.update({
          where: { id: workflowId },
          data: {
            name: versionData.name ?? undefined,
            description: versionData.description ?? undefined,
            nodes: versionData.nodes as Prisma.InputJsonValue,
            edges: versionData.edges as Prisma.InputJsonValue,
            variables: versionData.variables as Prisma.InputJsonValue,
            settings: versionData.settings as Prisma.InputJsonValue,
          },
        });

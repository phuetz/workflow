...
export class WorkflowRepository {
  ...
  async findById(id: string, userId?: string): Promise<Workflow | null> {
    ...
    if (userId && workflow.createdBy !== userId) {
      const userWorkflowIds = this.userWorkflows.get(userId);
  ...
  async updateStatistics(
    ...
    const totalTime = (stats.averageExecutionTime * (stats.totalExecutions - 1)) + execution.duration;
  ...
  async export(id: string, userId: string): Promise<Record<string, unknown> | null> {
    ...
    const exportData = { ...workflow } as Record<string, unknown>;
  ...
  async import(
    ...
    return this.create({
      ...
      createdBy: userId,
      updatedBy: userId
    } as Omit<Workflow, 'id' | 'createdAt' | 'updatedAt' | 'statistics'>, userId);
  }

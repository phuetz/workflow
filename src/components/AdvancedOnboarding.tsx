...
export default function AdvancedOnboarding() {
  ...
  const onboardingSteps: OnboardingStep[] = [
    ...
    },
    {
      ...
      action: () => {
        ...
        setTimeout(() => {
          const closeBtn = document.querySelector('.fixed .text-gray-500');
  ...
  useEffect(() => {
    ...
    const hasSeenOnboarding = localStorage.getItem('workflowbuilder_onboarding_completed');
  ...
  const addTourAttributes = () => {
    ...
    const aiGenerator = document.querySelector('button:has([data-lucide="sparkles"])');
    if (aiGenerator) aiGenerator.setAttribute('data-tour', 'ai-generator');
    
    const executeBar = document.querySelector('[class*="top-center"]');
    if (executeBar) executeBar.setAttribute('data-tour', 'execute-bar');
    
    const optimizer = document.querySelector('button:has([data-lucide="zap"])');
    if (optimizer) optimizer.setAttribute('data-tour', 'optimizer');
    
    const plugins = document.querySelector('button:has([data-lucide="package"])');
    if (plugins) plugins.setAttribute('data-tour', 'plugins');
    
    const voice = document.querySelector('button:has([data-lucide="message-circle"])');
    if (voice) voice.setAttribute('data-tour', 'voice');
    
    const collaboration = document.querySelector('button:has([data-lucide="users"])');
      ...
      <div className="fixed inset-0 bg-black bg-opacity-60 z-50">
        ...
        {currentStepData.highlight && getCurrentStepElement() && (
          <div
            ...
            style={{
              top: getCurrentStepElement()?.getBoundingClientRect().top! - 8,
              left: getCurrentStepElement()?.getBoundingClientRect().left! - 8,
              width: getCurrentStepElement()?.getBoundingClientRect().width! + 16,
              height: getCurrentStepElement()?.getBoundingClientRect().height! + 16,

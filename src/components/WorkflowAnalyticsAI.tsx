...
export default function WorkflowAnalyticsAI() {
  ...
  const analyzeWorkflow = async () => {
    ...
    try {
      ...
      const generatedInsights = generateWorkflowInsights();
      setInsights(generatedInsights);
      
      // Generate performance metrics
      const performanceMetrics = analyzeNodePerformance();
      ...
      addLog({
        level: 'info',
        message: 'Analyse IA du workflow complétée',
        data: { 
  ...
  const generateWorkflowInsights = (): AIInsight[] => {
    ...
    if (nodes.length > 0) {
      ...
      const connectedNodes = new Set(edges.map(e => e.target));
      ...
      const heavyOpNodes = nodes.filter(node => 
        ['code', 'python', 'openai', 'mongodb', 'mysql', 'postgres'].includes(node.data.type)
      );
      
      const nodesWithErrorHandling = new Set();
      edges.forEach(edge => {
      ...
      const heavyNodesWithoutErrorHandling = heavyOpNodes.filter(
  ...
  const analyzeNodePerformance = (): PerformanceMetric[] => {
    // Generate performance metrics for each node
    return nodes.map(node => {
      // Get execution results for this node
      const nodeResults = Object.entries(executionResults)
        .filter(([id]) => id === node.id)
        .map(([, result]) => result);
      
      // Calculate average execution time (simulated)
      const execTime = nodeResults.length > 0
        ? nodeResults.reduce((sum, result) => sum + (result.duration || 0), 0) / nodeResults.length
        : Math.random() * 500 + 100; // Random time if no data
      
      // Calculate success rate (simulated)
      const successRate = nodeResults.length > 0
        ? nodeResults.filter(result => result.status === 'success').length / nodeResults.length
        : 0.8 + Math.random() * 0.2; // Random success rate if no data
      ...
      return {
        ...
        bottleneck: execTime > 1000 // Mark as bottleneck if execution time > 1000ms
      };
    });
  };

  const generatePredictiveModel = (): PredictiveModel => {
    // Generate a predictive model based on execution history
    const executionTimes = executionHistory.map(exec => exec.duration || 0);
    
    // Calculate trend
    let trend: 'increasing' | 'decreasing' | 'stable' = 'stable';
    if (executionTimes.length >= 2) {
      const recentAvg = executionTimes.slice(-3).reduce((a, b) => a + b, 0) / 3;
      const olderAvg = executionTimes.slice(0, 3).reduce((a, b) => a + b, 0) / 3;
      
      if (recentAvg > olderAvg * 1.2) trend = 'increasing';
      else if (recentAvg < olderAvg * 0.8) trend = 'decreasing';
    }
    
    // Generate forecast (next 7 executions)
    const lastValue = executionTimes.length > 0 
      ? executionTimes[executionTimes.length - 1] 
      : 1000;
    
    const forecast = Array(7).fill(0).map((_, i) => {
      if (trend === 'increasing') return lastValue * (1 + (i + 1) * 0.05);
      if (trend === 'decreasing') return lastValue * (1 - (i + 1) * 0.03);
      return lastValue + (Math.random() * 200 - 100);
    });
    
    // Identify anomalies (simulated)
    const anomalies = [2, 5]; // Indices in the forecast array
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className={`p-4 rounded-lg ${
                    ...
                    {/* Calculate health score based on insights and metrics */}
                    {(() => {
                  ...
                  <div className={`p-4 rounded-lg ${
                    ...
                    <ul className="space-y-2 text-sm">
                      ...
                      <li className="flex items-start">
                        <CheckCircle size={16} className="text-green-500 mr-2 mt-0.5 flex-shrink-0" />
                        <span>Ajouter des mécanismes de retry avec backoff exponentiel pour les services instables</span>
                      </li>
                      <li className="flex items-start">
                        <CheckCircle size={16} className="text-green-500 mr-2 mt-0.5 flex-shrink-0" />
                        <span>Optimiser les transformations de données avec des opérations batch</span>
                      </li>
                      <li className="flex items-start">
                        <CheckCircle size={16} className="text-green-500 mr-2 mt-0.5 flex-shrink-0" />
                        <span>Monitorer les coûts API et implémenter des mécanismes de throttling</span>
                      </li>
                    </ul>
                  </div>
                </div>

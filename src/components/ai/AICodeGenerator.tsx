...
import { Code, Sparkles, Copy, Download, Zap, Brain } from 'lucide-react';
...
function processData(data) {
  ...
  const copyCode = async () => {
    // BROWSER COMPATIBILITY FIX: Safe clipboard access with fallback
    try {
      if (navigator.clipboard && navigator.clipboard.writeText) {
        ...
        if (success) {
          ...
          showNotification('Impossible de copier automatiquement. Veuillez copier manuellement.', 'warning');
        }
      }
    } catch (error) {
      logger.warn('Clipboard operation failed:', error);
      showNotification('Erreur lors de la copie. Veuillez copier manuellement.', 'error');
    }
  };
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <select
                      value={complexity}
                      onChange={(e) => setComplexity(e.target.value as 'simple' | 'intermediate' | 'advanced')}
                      className={`w-full px-3 py-2 border rounded-md ${
                        darkMode
                          ? 'bg-gray-700 border-gray-600 text-white'
                          : 'bg-white border-gray-300'

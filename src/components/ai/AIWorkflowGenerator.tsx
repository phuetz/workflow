...
// Removed unused AIGenerationRequest interface
...
export default function AIWorkflowGenerator() {
  ...
  const popularServices = [
    'Gmail', 'Slack', 'Discord', 'Notion', 'Airtable', 'OpenAI',
    'GitHub', 'Stripe', 'MySQL', 'AWS S3'
  ];


  const getTriggerConfig = (type: string, prompt: string) => {
    // Simplified trigger configuration with prompt context
    switch (type) {
      case 'schedule':
  ...
  const generateWorkflowFromPrompt = async (
    ...
    const triggerNodeId = `node_${nodeId}`;
    nodes.push({
      id: triggerNodeId,
      type: 'custom',
    ...
    nodeId++;
    let sourceId = triggerNodeId;

    // 2. Ajouter les nœuds de traitement selon le prompt
    const processingNodes = generateNodes(prompt);
    ...
    if (errorHandling && complexity !== 'simple') {
      const errorNodeId = `node_error_${nodeId}`;
      nodes.push({
        id: errorNodeId,
        type: 'custom',
    ...
    const estimatedCost = complexity === 'simple' ? 0.01 : complexity === 'medium' ? 0.05 : 0.1;
    const reliability = errorHandling ? 0.95 : 0.85;
  ...
  const generateNodes = (prompt: string) => {
    // Simplified node determination based on prompt only
    const promptLower = prompt.toLowerCase();
    const nodes: Record<string, unknown>[] = [];
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="space-y-2 max-h-40 overflow-y-auto">
                    {generatedWorkflow.nodes.map((node: any, index) => (
                      <div key={node.id as string} className="flex items-center space-x-3 p-2 bg-gray-50 rounded">
                        <div className="w-6 h-6 bg-blue-500 text-white rounded-full flex items-center justify-center text-xs">
                          {index + 1}
                        </div>
                        <div>
                          <div className="font-medium text-sm">{node.data?.label || 'Node'}</div>
                          <div className="text-xs text-gray-500">{node.data?.type || 'Unknown'}</div>
                        </div>
                      </div>
                    ))}
                  </div>

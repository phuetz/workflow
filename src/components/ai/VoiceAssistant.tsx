...
import { useSafeSpeechRecognition, useBrowserCapabilities } from '../../hooks/useBrowserCompatibility';
...
export default function VoiceAssistant() {
  ...
  // CROSS-BROWSER FIX: Use safe speech recognition hook
  const speechRecognition = useSafeSpeechRecognition();
  ...
  // MEMORY LEAK FIX: Cleanup speech synthesis on unmount
  useEffect(() => {
    return () => {
      ...
      if (typeof speechSynthesis !== 'undefined') {
        try {
          ...
          logger.warn('Speech synthesis cleanup failed:', error);
        }
      }
  ...
  const voiceCommands: VoiceCommand[] = [
    {
      ...
      action: () => {
        const status = isExecuting ? 'en cours d\'exécution' : 'inactif';
    ...
    },
    {
      ...
      action: () => {
        ...
        if (lastExecution) {
          const status = lastExecution.status === 'success' ? 'réussi' : 'échoué';
    ...
    },
    {
      command: "mode sombre",
      action: () => {
        const currentDarkMode = useWorkflowStore.getState().darkMode;
        useWorkflowStore.getState().toggleDarkMode();
        const newMode = currentDarkMode ? 'clair' : 'sombre';
  ...
  const handleCommand = useCallback((command: string) => {
    ...
    const matchedCommand = voiceCommands.find(cmd => 
      ...
      cmd.command.includes(normalizedCommand)
    );
    ...
    if (matchedCommand) {
      ...
      addLog({
        ...
        data: { command: matchedCommand.command, transcript: command }
      });
    } else {
    ...
    setTranscript('');
  }, [voiceCommands, speak, nodes, addLog]);
  ...
  }, [transcript, isListening, handleCommand]);

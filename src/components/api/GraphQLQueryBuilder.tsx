...
import { graphqlSchemaService, GraphQLIntrospectionResult } from '../../services/GraphQLSchemaService';
...
// Convert GraphQLIntrospectionResult to GraphQLSchema format
const convertIntrospectionToSchema = (introspection: GraphQLIntrospectionResult): GraphQLSchema => {
  const schema: GraphQLSchema = { types: {} };
  
  Object.entries(introspection.types).forEach(([typeName, fields]) => {
    schema.types[typeName] = fields.map(field => ({
      ...field,
      args: field.args || [],
      fields: field.fields || []
    }));
...
export default function GraphQLQueryBuilder() {
  ...
  // Get demo schema from service for fallback
  const getDemoSchema = () => {
    const demoIntrospection = graphqlSchemaService.createDemoSchema();
    return convertIntrospectionToSchema(demoIntrospection);
  };

  // Initialize demo schema
  const demoSchema = getDemoSchema();

  const loadSchema = async () => {
    if (!schemaUrl.trim()) {
      notificationService.show('error', 'Erreur', 'Veuillez entrer une URL de schéma GraphQL');
      return;
    }
    ...
    try {
      ...
      if (schemaUrl === 'service://internal') {
        // Load from internal GraphQL service
        introspectionResult = await graphqlSchemaService.getSchemaFromService();
        addLog({
          level: 'info',
          message: 'Schéma GraphQL interne chargé avec succès',
          data: { source: 'internal', types: Object.keys(introspectionResult.types).length }
        });
      } else {
        // Load from external URL
        introspectionResult = await graphqlSchemaService.fetchSchemaFromUrl(schemaUrl);
        addLog({
          level: 'info',
          message: 'Schéma GraphQL chargé avec succès',
          data: { url: schemaUrl, types: Object.keys(introspectionResult.types).length }
        });
      }
      
      // Convert to legacy format and set schema
      const legacySchema = convertIntrospectionToSchema(introspectionResult);
      setSchema(legacySchema);
      
      notificationService.show('success', 'Succès', 'Schéma GraphQL chargé avec succès');
      ...
      addLog({
        ...
        data: { error: errorMessage, url: schemaUrl }
      });
      
      notificationService.show('error', 'Erreur', `Échec du chargement: ${errorMessage}`);
    } finally {
      setIsLoading(false);
    }
  };

  const toggleFieldSelection = (field: GraphQLField, subFieldIndex?: number) => {
    if (!schema) return;
  ...
  const toggleField = (typeName: string, fieldIndex: number, subFieldIndex?: number) => {
    if (!schema || !schema.types[typeName]) return;

    const field = schema.types[typeName][fieldIndex];
    if (!field) return;
  ...
  const generateQuery = () => {
    ...
    const selectedFields = schema.types[selectedType]?.filter(f => f.selected) || [];
    ...
    const query = `${selectedType.toLowerCase()} {\n  ${queryFields.join('\n  ')}\n}`;
  ...
  const formatArgValue = (value: unknown, type: string): string => {
    ...
    if (type.includes('String')) {
      ...
      // IDs should also be escaped like strings
      return `"${escapedValue}"`;
    }
    // For unknown types, return as string but escaped
    return `"${escapedValue}"`;
  };

  const updateArgValue = (field: GraphQLField, argIndex: number, value: string) => {
    if (!schema) return;
  ...
  const updateArgument = (typeName: string, fieldIndex: number, argIndex: number, value: string) => {
    if (!schema || !schema.types[typeName]) return;

    const field = schema.types[typeName][fieldIndex];
    if (!field || !field.args || !field.args[argIndex]) return;
  ...
  const executeQuery = async () => {
    if (!generatedQuery.trim()) {
      notificationService.show('warning', 'Attention', 'Veuillez générer une requête avant l\'exécution');
      return;
    }
    
    if (!schemaUrl.trim()) {
      notificationService.show('error', 'Erreur', 'Aucune URL de schéma configurée');
      return;
    }
    ...
    try {
      ...
      let result;
      if (schemaUrl === 'service://internal') {
        // For internal service, we can't execute queries directly, show a demo result
        result = {
          data: {
            message: "Exemple de résultat pour le service interne",
            timestamp: new Date().toISOString()
          }
        };
        notificationService.show('info', 'Info', 'Simulation d\'exécution pour le service interne');
      } else {
        // Execute query against external endpoint
        result = await graphqlSchemaService.executeQuery(schemaUrl, generatedQuery);
      }
      ...
      addLog({
        ...
        data: { 
          query: generatedQuery.substring(0, 100) + (generatedQuery.length > 100 ? '...' : ''),
          result: result,
          url: schemaUrl 
        }
      });

      notificationService.show('success', 'Succès', 'Requête exécutée avec succès');

      // Validate the query result
      if (result.errors && result.errors.length > 0) {
        const errorMessages = result.errors.map((e: { message: string }) => e.message).join(', ');
        notificationService.show('warning', 'Attention', `Requête exécutée avec des erreurs: ${errorMessages}`);
      }
      ...
      addLog({
        ...
        data: { error: errorMessage, query: generatedQuery, url: schemaUrl }
      });

      notificationService.show('error', 'Erreur', `Échec de l'exécution: ${errorMessage}`);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const copyQuery = async () => {
    ...
    notificationService.show('success', 'Succès', 'Requête copiée dans le presse-papiers');
  };
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="https://api.example.com/graphql ou service://internal"
                ...
                <div className="mt-4">
                  ...
                  <div className="space-y-2">
                    <button
                      onClick={() => setSchemaUrl('service://internal')}
                      className={`w-full p-2 text-left rounded border transition-colors ${
                        schemaUrl === 'service://internal' 
                          ? (darkMode ? 'border-blue-500 bg-blue-900/20' : 'border-blue-500 bg-blue-50')
                          : (darkMode ? 'border-gray-600 hover:border-gray-500' : 'border-gray-300 hover:border-gray-400')
                      }`}
                    >
                      <div className="text-sm font-medium">Service GraphQL Interne</div>
                      <div className="text-xs text-gray-500">Schéma workflow local</div>
                    </button>
                    {graphqlSchemaService.getPopularEndpoints().map(example => (
                      <button
                        key={example.name}
                        onClick={() => setSchemaUrl(example.url)}
                        className={`w-full p-2 text-left rounded border transition-colors ${
                          schemaUrl === example.url
                            ? (darkMode ? 'border-blue-500 bg-blue-900/20' : 'border-blue-500 bg-blue-50')
                            : (darkMode ? 'border-gray-600 hover:border-gray-500' : 'border-gray-300 hover:border-gray-400')
                        }`}
                      >
                        <div className="text-sm font-medium flex items-center justify-between">
                          ...
                          {example.requiresAuth && (
                            <span className="text-xs px-1.5 py-0.5 bg-yellow-100 text-yellow-800 rounded">Auth</span>
                          )}
                        </div>
                ...
                {/* Use Demo Schema Button */}
                <button
                  onClick={() => {
                    setSchema(demoSchema);
                    setSchemaUrl('demo://local');
                    notificationService.show('info', 'Info', 'Schéma de démonstration chargé');
                    addLog({
                      level: 'info',
                      message: 'Schéma de démonstration chargé',
                      data: { types: Object.keys(demoSchema.types).length }
                    });
                  }}

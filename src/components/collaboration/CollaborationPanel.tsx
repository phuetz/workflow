...
interface CollaboratorWithDetails {
  id: string;
  name: string;
  email: string;
...
export const CollaborationPanel = memo<CollaborationPanelProps>(({ isOpen, onClose }) => {
  const {
    collaborators: storeCollaborators,
    darkMode
  } = useWorkflowStore();

  // Transform collaborators from store format to expected format
  const collaborators: CollaboratorWithDetails[] = storeCollaborators.map((collab, index) => ({
    id: `collab-${index}`,
    name: collab.email.split('@')[0],
    email: collab.email,
    permission: (collab.permissions.includes('admin') ? 'admin' : collab.permissions.includes('write') ? 'edit' : 'view') as 'view' | 'edit' | 'admin',
    isOnline: true
  }));

  // Mock comments functionality (to be implemented in store later)
  ...
  const updateCollaborator = useCallback((userId: string, updates: { permission: string }) => {
    logger.info('Updating collaborator:', { userId, updates });
  ...
  const handleSendMessage = useCallback(() => {
    if (newMessage.trim()) {
      ...
      logger.info('Sending message:', newMessage);
      setNewMessage('');
    }
  }, [newMessage]);
              ...
              <div key={collaborator.id} className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <select
                    ...
                    onChange={(e) => handlePermissionChange(collaborator.id, e.target.value as 'view' | 'edit' | 'admin')}

...
export default function ExpressionBuilder() {
  ...
  const evaluateExpression = () => {
    try {
      ...
      evalResult = evalResult.replace(/\{\{\s*\$uuid\s*\}\}/g, (() => {
        const randomStr = Math.random().toString(36).substring(2, 11);
        return 'uuid-' + (randomStr.length >= 9 ? randomStr.substring(0, 9) : randomStr.padEnd(9, '0'));
      })());
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className={`${darkMode ? 'bg-gray-800' : 'bg-gray-50'} rounded-lg p-4`}>
            <h2 className="font-semibold mb-4">
              {categories[selectedCategory as keyof typeof categories].name} Functions
            </h2>
            <div className="space-y-3">
              {categories[selectedCategory as keyof typeof categories].functions.map((func, index) => (
                <div key={`example-${index}`} className={`p-3 rounded-lg border ${darkMode ? 'border-gray-700' : 'border-gray-200'}`}>
                  <div className="flex items-center justify-between mb-2">
                    <code className="font-mono text-sm font-bold text-purple-500">
                      {func.name}
                    </code>
                    <button
                      onClick={() => insertExample(func.example)}

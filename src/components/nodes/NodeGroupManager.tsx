...
export const NodeGroupManager: React.FC = () => {
  ...
  const handleGroupNodes = () => {
    ...
    // WorkflowNode doesn't have width/height properties, using default node dimensions
    const nodeWidth = 200;
    const nodeHeight = 100;
    const maxX = Math.max(...selectedNodeData.map(n => n.position.x + nodeWidth));
    const maxY = Math.max(...selectedNodeData.map(n => n.position.y + nodeHeight));

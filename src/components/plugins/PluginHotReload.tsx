...
export default function PluginHotReload() {
  ...
  const loadPluginInSandbox = async (plugin: Plugin) => {
    ...
    logger.info(`🔒 Plugin "${plugin.name}" loaded in secure sandbox`);
    logger.info('📦 Available methods:', plugin.config.methods);
    logger.info('🛡️ Security context: Isolated VM with limited permissions');
  };

 ...
 * Browse, preview, and install workflow templates
 */
...
export const WorkflowTemplates: React.FC<WorkflowTemplatesProps> = ({
  ...
  const handleInstall = async (template: WorkflowTemplate) => {
    try {
      ...
      // Show success message or redirect to workflow editor
      setTimeout(() => {
        setInstalling(null);
      }, 2000);
    } catch (error) {
        ...
        <div className="p-6">
          ...
          <div className="flex space-x-2">
            ...
            <button
              ...
              className="flex-1 px-3 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 disabled:bg-blue-400 text-sm flex items-center justify-center"
            >
              {isInstalling ? (
                <>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center flex-1">
            ...
            <div className="flex-1">
              <div className="flex items-center mb-1">
                <h3 className="font-semibold text-gray-900 mr-2">{template.name}</h3>
                {template.featured && <Award className="w-4 h-4 text-yellow-500 mr-2" />}
                {getAuthorBadge(template.authorType)}
                {template.pricing !== 'free' && (
          ...
          <div className="flex space-x-2 ml-4">
            ...
            <button
              onClick={() => handleInstall(template)}
              disabled={isInstalling}
              className="px-3 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 disabled:bg-blue-400 text-sm flex items-center"
            >
              {isInstalling ? (
                <>
                  <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white mr-1"></div>

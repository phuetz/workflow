/**
 * Advanced Analytics & Business Intelligence Service - Layer 4: Reporting
 *
 * Responsibilities:
 * - Event tracking (user, workflow, system events)
 * - Complex analytics queries with caching
 * - Dashboard management and data fetching
 ...
 *
 * Use Cases:
 * - Track events: advancedAnalyticsService.track('event_type', properties)
 * - Complex queries: advancedAnalyticsService.query(analyticsQuery)
 * - Dashboard data: advancedAnalyticsService.getDashboardData(dashboardId)
 * - Funnel analysis: advancedAnalyticsService.createFunnel(funnelConfig)
 * - Cohort analysis: advancedAnalyticsService.createCohort(cohortConfig)
 * - Scheduled reports: advancedAnalyticsService.generateReport(reportId)
 *
 * Note: For simple aggregated metrics, use AnalyticsService.
 * For recording workflow executions, use WorkflowAnalyticsService.
 * For time-series storage, use AnalyticsPersistence.
 *
...
export class AdvancedAnalyticsService extends EventEmitter {
  ...
  public track(
    ...
    const event: AnalyticsEvent = {
      ...
      workflowId: properties.workflowId as string | undefined,
      nodeId: properties.nodeId as string | undefined,
      timestamp: new Date(),
  ...
  public async query(query: AnalyticsQuery): Promise<AnalyticsResult> {
    ...
    const cached = await cacheLayer.get<AnalyticsResult>(cacheKey);
  ...
  private async executeQuery(query: AnalyticsQuery): Promise<AnalyticsResult> {
    ...
    const groupedData = query.groupBy ? this.groupEventsByDimensions(filteredEvents, query.groupBy) : new Map([['all', filteredEvents]]);

    // Calculate metrics
    const data = this.calculateMetrics(groupedData, query.metrics);

    // Apply ordering
    if (query.orderBy) {
      data.sort((a, b) => {
        for (const orderBy of query.orderBy!) {
          const aValue = a[orderBy.field] as number;
          const bValue = b[orderBy.field] as number;
  ...
  public async saveDashboard(dashboard: Dashboard): Promise<void> {
    ...
    await cacheLayer.invalidateByTags([`dashboard:${dashboard.id}`]);
  ...
  public async getDashboardData(
    ...
    for (const widget of dashboard.widgets) {
      try {
        ...
        data[widget.id] = { error: (error as Error).message };
      }
    }
  ...
  private async getWidgetData(widget: Widget, globalFilters: Record<string, unknown>): Promise<unknown> {
    const cacheKey = widget.dataSource.cacheKey || `widget:${widget.id}:${JSON.stringify({ ...widget.filters, ...globalFilters })}`;
  ...
  public async analyzeFunnel(funnelId: string): Promise<FunnelResult> {
    ...
    const totalUsers = stepResults.length > 0 ? stepResults[0].users.size : 0;
    const steps = stepResults.map((step, index) => {
      const users = step.users.size;
      const percentage = totalUsers > 0 ? (users / totalUsers) * 100 : 0;
      const dropoff = index > 0 ? stepResults[index - 1].users.size - users : 0;
  ...
  public async analyzeCohort(cohortId: string): Promise<CohortResult> {
    ...
    const definitionEvents = this.events.filter(event => {
    ...
    definitionEvents.forEach(event => {
      ...
      const period = this.getPeriodKey(event.timestamp, cohort.analysis.periodType);
    ...
    const maxPeriods = cohort.analysis.periods;

    for (const [period, users] of Array.from(cohortGroups.entries())) {
      const retention: number[] = [];
      const cohortSize = users.size;

      for (let i = 0; i < maxPeriods; i++) {
        const periodStart = this.addPeriods(new Date(period), i, cohort.analysis.periodType);
        const targetPeriodKey = this.getPeriodKey(periodStart, cohort.analysis.periodType);
        ...
        this.events.forEach(event => {
          ...
          const eventPeriod = this.getPeriodKey(event.timestamp, cohort.analysis.periodType);
    ...
    for (let i = 0; i < maxPeriods; i++) {
      const periodRetentions = cohorts.map(c => c.retention[i]);
      const average = periodRetentions.length > 0
        ? periodRetentions.reduce((sum, val) => sum + val, 0) / periodRetentions.length
        : 0;
  ...
  public async generateReport(reportId: string): Promise<{
    ...
    for (const query of report.queries) {
      try {
        const result = await this.executeRawQuery(query.query, query.parameters);
        data.push({
          name: query.name,
          data: result
        });
  ...
  public getRealtimeMetrics(): {
    ...
    const executionsPerMinute = recentEvents.filter(
      event => event.type === 'workflow_executed'
    ).length;

    // Error rate (last 5 minutes)
    const executions = recentEvents.filter(event => event.type === 'workflow_executed');
    const failedExecutions = executions.filter(event => event.properties.status === 'failed');
    const errorRate = executions.length > 0 ? (failedExecutions.length / executions.length) * 100 : 0;

    // Average execution time (last 5 minutes)
    const executionTimes = recentEvents
  ...
  private flushEventBuffer(): void {
    ...
    // Record metrics (MonitoringService doesn't have recordMetric, so we log instead)
    logger.debug(`Processed ${this.events.length} analytics events`);
  }
  ...
  private async processScheduledReports(): Promise<void> {
    ...
    for (const report of Array.from(this.reports.values())) {
      if (report.type === 'scheduled' && report.nextRun && report.nextRun <= now) {
        try {
          const result = await this.generateReport(report.id);
  ...
  private groupEventsByDimensions(events: AnalyticsEvent[], dimensions: string[]): Map<string, AnalyticsEvent[]> {
    ...
    for (const event of events) {
      const key = dimensions.map(dim => this.getFieldValue(event, dim)).join('|');
  ...
  private calculateMetrics(groupedData: Map<string, AnalyticsEvent[]>, metrics: string[]): Array<Record<string, unknown>> {
    ...
    for (const [key, events] of Array.from(groupedData.entries())) {

/**
 * Analytics Service - Layer 3: Aggregation
 *
 * Responsibilities:
 * - Aggregate workflow and node metrics
 * - Manage alert rules and trigger alerts
 * - Generate performance reports
 * - Provide real-time monitoring endpoints
 * - Export data to various formats
 *
 * Use Cases:
 * - Get aggregated workflow metrics: analyticsService.getWorkflowMetrics()
 * - Create alert rules: analyticsService.createAlertRule()
 * - Generate reports: analyticsService.generatePerformanceReport()
 * - Real-time dashboard: analyticsService.getRealTimeMetrics()
 *
 * Note: For BI dashboards and funnel/cohort analysis, use AdvancedAnalyticsService.
 * For recording individual executions, use WorkflowAnalyticsService.
 *
...
export class AnalyticsService {
  ...
  async exportData(
    ...
    if (format === 'csv') {
      data = this.convertToCSV(data as unknown[]);
    } else if (format === 'xlsx') {
      data = this.convertToExcel(data as unknown[]);
    }

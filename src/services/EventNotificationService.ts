/**
 * Event Notification Service - Layer 2: Event-Based Notifications
 *
 * Responsibilities:
 * - Define notification rules triggered by application events
 * - Support conditions for selective notification
 * - Cooldown to prevent notification spam
 * - Priority-based notification handling
 * - Event history tracking
 *
 * Use Cases:
 * - Emit event: eventNotificationService.emit('workflow_execution_completed', data)
 * - Add rule: eventNotificationService.addRule(rule)
 * - Enable/disable: eventNotificationService.setRuleEnabled(ruleId, enabled)
 *
 * Built-in Rules:
 * - workflow_execution_success/failed
 * - test_execution_completed
 * - quota_warning
 * - template_added
 *
 * Note: This service uses NotificationService (Layer 1) for display.
 * For direct UI notifications, use NotificationService.
 * For multi-channel delivery, use UnifiedNotificationService.
 *
...
export class EventNotificationService extends EventEmitter {
  ...
  private initializeDefaultRules = (): void => {
    const defaultRules: NotificationRule[] = [
      ...
      },
      {
        ...
        condition: (data) => (data.percentage as number) >= 80,
      ...
      },
      {
        ...
        condition: (data) => (data.duration as number) > 5000, // > 5 seconds
        notificationType: 'warning',
        title: 'Slow Node Execution',
        message: (data) => `Node "${data.nodeName}" took ${Math.round((data.duration as number) / 1000)}s to execute`,
        enabled: true,
        cooldown: 60000,
        priority: 'low'
      },
      {
        ...
        condition: (data) => (data.remainingRequests as number) < 10,
        notificationType: 'warning',
  ...
  private processEventForNotifications(eventData: AppEventData): void {
    ...
    for (const rule of matchingRules) {
      ...
      if (rule.condition && !rule.condition(eventData.data as Record<string, unknown>)) {
        continue;
      }
  ...
  private generateNotification(rule: NotificationRule, eventData: AppEventData): void {
    const message = typeof rule.message === 'function'
      ? rule.message(eventData.data as Record<string, unknown>)
      : rule.message;

    // Create context-aware actions based on event type
    const actions = this.createNotificationActions(rule, eventData);
  ...
  private createNotificationActions(rule: NotificationRule, eventData: AppEventData): Array<{label: string; action: () => void}> {

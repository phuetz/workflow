...
export class GraphQLSchemaService {
  ...
  async fetchSchemaFromUrl(url: string, headers: Record<string, string> = {}): Promise<GraphQLIntrospectionResult> {
    try {
      ...
      const result = await response.json() as { data?: { __schema: unknown }; errors?: Array<{ message: string }> };
      ...
      const introspection = this.processIntrospectionResult(result.data.__schema);
  ...
  async getSchemaFromService(): Promise<GraphQLIntrospectionResult> {
    try {
      ...
      const { GraphQLService } = await import('./GraphQLService');
      ...
      const graphqlService = new GraphQLService(
        mockServices.rbac as any,
        mockServices.secrets as any,
        mockServices.llm as any,
        mockServices.marketplace as any,
        mockServices.plugins as any
      );

      const result = await graphqlService.introspect() as {
        data?: { __schema: any };
  ...
  private processIntrospectionResult(schema: any): GraphQLIntrospectionResult {
    ...
    if (schema.types) {
      schema.types.forEach((type: any) => {
        ...
        if (type.kind === 'OBJECT' && type.fields) {
          const fields = type.fields.map((field: any) => this.processField(field));
    ...
    if (schema.directives) {
      schema.directives.forEach((directive: any) => {
        directives.push({
          name: directive.name,
  ...
  private getTypeFields(schema: any, typeName: string): GraphQLSchemaField[] {
    const type = schema.types?.find((t: any) => t.name === typeName);
    if (!type || !type.fields) return [];

    return type.fields.map((field: any) => this.processField(field));
  }

  private processField(field: any): GraphQLSchemaField {
    return {
      name: field.name,
      type: this.formatType(field.type),
      description: field.description || '',
      args: field.args ? field.args.map((arg: any) => this.processArgument(arg)) : [],
  ...
  private processArgument(arg: any): GraphQLSchemaArgument {
    return {
      name: arg.name,
  ...
  async executeQuery(
    ...
    try {
      ...
      const cached = this.executionCache.get(cacheKey) as { timestamp: number; result: unknown } | undefined;
  ...
  validateQuery(query: string, schema?: GraphQLIntrospectionResult): SchemaValidationResult {
    ...
    try {
      ...
      const queryType = query.trim().split(/\s+/)[0];
      ...
      if (schema && result.isValid) {
        ...
        const rootType = queryType === 'mutation' ? 'Mutation' :
                        queryType === 'subscription' ? 'Subscription' : 'Query';

        if (schema.types[rootType]) {
          const queryFields = this.extractFieldsFromQuery(query);
          const availableFields = schema.types[rootType].map(f => f.name);
  ...
  private extractFieldsFromQuery(query: string): string[] {
    ...
    const fieldMatches = query.match(/\b(\w+)\s*(?:\(|{)/g) || [];
    return fieldMatches
      .map(match => match.replace(/\s*[({].*$/, ''))
      .filter(field => !['query', 'mutation', 'subscription'].includes(field.toLowerCase()));
  }
  ...
  compareSchemas(schema1: GraphQLIntrospectionResult, schema2: GraphQLIntrospectionResult): {
    ...
    // Compare types
    const oldSchema = schema1;
    const newSchema = schema2;
    const types1 = Object.keys(oldSchema.types || {});
    const types2 = Object.keys(newSchema.types || {});
    ...
    types1.forEach(typeName => {
      if (types2.includes(typeName)) {
        const fields1 = oldSchema.types[typeName].map(f => f.name);
        const fields2 = newSchema.types[typeName].map(f => f.name);
        const addedFields = fields2.filter(f => !fields1.includes(f));
        const removedFields = fields1.filter(f => !fields2.includes(f));

...
class OptimizationService extends EventEmitter {
  ...
  async optimizeWorkflow(
    ...
    try {
      ...
      const result = await this.applyOptimizations(nodes, edges, [], config);
  ...
  private async generateOptimizationCandidates(
    ...
    const mergeableGroups = this.findMergeableNodes(nodes, edges);
    const expensiveNodes = this.findExpensiveNodes(nodes);
    const batchableNodes = this.findBatchableNodes(nodes);
    const reorderSuggestions = this.suggestReordering(nodes, edges, analysis);
    ...
    const analysisData = analysis as { parallelizableGroups?: string[][] };
    if (analysisData.parallelizableGroups) {
      analysisData.parallelizableGroups.forEach((group: string[]) => {
        if (group.length > 1) {
          candidates.push({
            type: 'parallel',
  ...
  private calculateCandidateScore(candidate: OptimizationChange, config: OptimizationConfig): number {
    let score = candidate.estimatedSavings * candidate.confidence;
  ...
  private async applyOptimizations(
    ...
    const originalCost = this.calculateWorkflowCost(nodes, edges);
  ...
  private findExecutionPaths(nodes: WorkflowNode[], edges: WorkflowEdge[]): string[][] {
    ...
    startNodes.forEach(startNode => {
      const path = this.tracePath(startNode.id, edges, []);
    ...
    for (let i = 0; i < nodes.length; i++) {
      for (let j = i + 1; j < nodes.length; j++) {
        ...
        if (node1.type === node2.type &&
          ...
          const inputs1 = edges.filter(e => e.target === node1.id).map(e => e.source).sort();
          const inputs2 = edges.filter(e => e.target === node2.id).map(e => e.source).sort();
  ...
  private findExpensiveNodes(nodes: WorkflowNode[]): string[] {
    const expensiveTypes = ['database', 'httpRequest', 'computation', 'fileOperation'];
    return nodes
  ...
  private suggestReordering(
    ...
    const expensiveTypes = ['httpRequest', 'database', 'computation'];

    // Suggest moving expensive operations later in the pipeline
    const earlyExpensive = nodes
      .filter(node => expensiveTypes.includes(node.type))
      .filter(node => {
        const dependencies = edges.filter(e => e.target === node.id);
  ...
  private calculateWorkflowCost(nodes: WorkflowNode[], edges: WorkflowEdge[]): number {
    ...
    const baseCost = nodes.length * 10;
    const complexityCost = edges.length * 5;
    const typeCost = nodes.reduce((total, node) =>
  ...
  private estimateLatency(
    ...
    const expensiveNodes = this.findExpensiveNodes(nodes);
    let baseLatency = nodes.length * 100;

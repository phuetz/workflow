...
export class PluginEngine {
  ...
  private async executeSandboxed(
    ...
    return new Promise((resolve, reject) => {
      ...
      if (!sandbox) {
        ...
        const workerCode = this.generateSandboxCode(plugin);
    ...
    return `
      ...
      self.onmessage = async function(event) {
        ...
        if (type === 'execute') {
          try {
            // Deserialize context
            const pluginContext = deserializeContext(context);

            // Execute plugin
            const result = await (${plugin.execute.toString()})(input, pluginContext);
        ...
        return {
          ...
          http: {
            ...
            post: async (url, data, options) => {
              const response = await fetch(url, { method: 'POST', body: JSON.stringify(data), ...options });
              return response.json();
            },
            put: async (url, data, options) => {
              const response = await fetch(url, { method: 'PUT', body: JSON.stringify(data), ...options });
              return response.json();
            },
  ...
  private createPluginContext(pluginId: string): PluginContext {
    const authService = { getCurrentUser: () => 'anonymous' };
  ...
  private async safeHttpRequest(url: string, options: any = {}): Promise<Response> {
    try {
      // Validate URL format

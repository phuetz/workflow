...
// Dynamic import for Bull to handle optional dependency - types only
type Queue = any;
type Job = any;
type JobOptions = any;
type ProcessorFunction<T> = (job: Job) => Promise<T>;
type QueueScheduler = any;

// Lazy-loaded Bull using ES module dynamic import
let _bull: any = null;
const getBull = async (): Promise<any> => {
  if (_bull !== null) return _bull;
  try {
    const module = await import('bull');
    _bull = module.default || module;
    return _bull;
  } catch {
    _bull = null;
    return null;
...
export class QueueWorkerService extends EventEmitter {
  ...
  public async createQueue(name: string, options: JobOptions = {}): Promise<Queue> {
    ...
    if (!Bull) {
      throw new Error('Bull queue library not available');
    }
    ...
    if (this.config.enableScheduler && Bull.QueueScheduler) {
      try {
        const scheduler = new Bull.QueueScheduler(name, {
          redis: this.config.redis,
        ...
        logger.warn(`Failed to create queue scheduler for ${name}:`, e);
      }
    }
  ...
  public registerProcessor(
    ...
    // Create worker using Bull's process method
    queue.process(concurrency || this.config.concurrency, async (job: Job) => {
    ...
    // Setup worker events
    this.setupQueueWorkerEvents(queue, queueName);
  ...
  public async getWorkerMetrics(queueName: string): Promise<WorkerMetrics | null> {
    const queue = this.workers.get(queueName);
  ...
  private setupQueueWorkerEvents(queue: Queue, name: string): void {
    // Worker events are handled through queue events in Bull
    queue.on('error', (error) => {
    ...
    queue.on('stalled', (job) => {
      this.emit('worker:stalled', { worker: name, jobId: job.id });
    });
  }

...
export class RealTimeCollaborationService extends EventEmitter {
  ...
  private setupWebSocketHandlers(): void {
    // WebSocket handlers are registered through the WebSocketServerService
    // This method is kept for future extensibility
    logger.debug('Setting up collaboration WebSocket handlers');
  }
  ...
  private handleWebSocketMessage(data: unknown): void {
    ...
    switch (type) {
      case 'collaboration:join':
        this.joinSession(userId, payload.workflowId, payload.userInfo);
        break;
      case 'collaboration:leave':
        this.leaveSession(userId, payload.sessionId);
        break;
  ...
  public async joinSession(
    ...
    logger.info(`👥 User ${userId} joined collaboration session for workflow ${workflowId}`);
  ...
  public async leaveSession(userId: string, workflowId: string): Promise<void> {
    ...
    logger.info(`👤 User ${userId} left collaboration session for workflow ${workflowId}`);
  ...
  public async handleComment(
    ...
    logger.info(`💬 Comment created by ${userId} on workflow ${workflowId}`);
  ...
  private async createSession(workflowId: string): Promise<CollaborationSession> {
    ...
    logger.info(`📝 Created collaboration session for workflow ${workflowId}`);
  ...
  private async cleanupSession(workflowId: string): Promise<void> {
    ...
    logger.info(`🧹 Cleaned up collaboration session for workflow ${workflowId}`);
  }
  ...
  private broadcastToSession(
    ...
    for (const [userId] of Array.from(session.participants)) {
      if (userId !== excludeUserId) {
        webSocketServerService.sendNotificationToUser(userId, {
          type: event,
          data: { sessionId, ...(data as Record<string, unknown>) }
        });
      }
    }
  }
  ...
  private operationsConflict(op1: Operation, op2: Operation): boolean {
    ...
    if (op1.type === op2.type) {
      ...
      switch (op1.type) {
        ...
        case 'edge_update':
        case 'edge_delete':
          return data1.edgeId === data2.edgeId;
        case 'property_update':
          return data1.nodeId === data2.nodeId && data1.property === data2.property;
      }
    }
    return false;
  }
  ...
  private async autoSave(session: CollaborationSession): Promise<void> {
    try {
      ...
      logger.error(`❌ Auto-save failed for session ${session.id}:`, error);
    }
  }

  private recordMetrics(event: string, data: Record<string, unknown>): void {
    // Convert data to string tags for monitoring service
    const tags: Record<string, string> = {};
    for (const [key, value] of Object.entries(data)) {
      tags[key] = String(value);
    }
    monitoringService.recordMetric(`collaboration.${event}`, 1, tags);
  }

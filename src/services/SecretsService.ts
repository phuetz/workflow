...
export class SecretsService {
  ...
  constructor() {
    // Initialize encryption key from environment or generate a secure default
   ...
   * Falls back to a derived key from available secrets for development
   */
  private getEncryptionKey(): string {
    // Try environment variables first (production)
    const envKey = typeof process !== 'undefined'
      ? process.env.SECRETS_ENCRYPTION_KEY || process.env.ENCRYPTION_KEY
      : undefined;
    ...
    const viteKey = typeof import.meta !== 'undefined'
      ? (import.meta.env?.VITE_SECRETS_ENCRYPTION_KEY || import.meta.env?.VITE_ENCRYPTION_KEY) as string | undefined
      : undefined;

    if (viteKey && viteKey.length >= 32) {
      return viteKey;
    }

    // Development fallback - derive from machine identifier
    // WARNING: This is only for development. Set proper env vars in production!
    if (typeof process !== 'undefined' && process.env.NODE_ENV !== 'production') {
      logger.warn('Using derived encryption key. Set SECRETS_ENCRYPTION_KEY in production!');
      // Create a deterministic key from hostname and user for development consistency
      const hostname = typeof process !== 'undefined' && process.env.HOSTNAME || 'localhost';
      const user = typeof process !== 'undefined' && process.env.USER || 'dev';
      const baseString = `workflow-dev-${hostname}-${user}-encryption-key`;
      // Simple hash to create a consistent 32-char key
      let hash = 0;
      ...
      return `dev-key-${Math.abs(hash).toString(16).padStart(8, '0')}-${Date.now().toString(16)}`.substring(0, 32);
    }
  ...
  async getSecret(secretId: string, userId?: string): Promise<Secret | null> {
    ...
    // Check permissions
    const hasAccess = await this.checkSecretAccess(secret, userId || this.currentUserId, 'read');
    if (!hasAccess) {
  ...
  async updateSecret(secretId: string, updates: Partial<Secret>): Promise<Secret | null> {
    ...
    // Check permissions
    const hasAccess = await this.checkSecretAccess(secret, this.currentUserId, 'write');
    if (!hasAccess) {
  ...
  async deleteSecret(secretId: string): Promise<boolean> {
    ...
    // Check permissions
    const hasAccess = await this.checkSecretAccess(secret, this.currentUserId, 'delete');
    if (!hasAccess) {
  ...
  private async executeRotationJob(job: SecretRotationJob): Promise<void> {
    try {
      job.status = 'running';
      ...
      const newValue = await this.generateNewSecretValue(job.secretId);

      // Step 3: Update secret
      const secret = this.secrets.get(job.secretId);
      if (!secret) {
        logger.error(`Secret ${job.secretId} not found for rotation`);
        return;
      }
      const oldValue = await this.decryptValue(secret.value);
      ...
      this.addRotationLog(job, 'error', `Rotation failed: ${errorMessage}`);
  ...
  private async executeValidationStep(job: SecretRotationJob, step: ValidationStep): Promise<void> {
  ...
  async validateSecretPolicy(secret: Secret): Promise<void> {
    ...
    for (const policy of policies) {
      ...
      for (const rule of policy.rules) {
        ...
        const violation = await this.evaluatePolicyRule(rule, secret);
        if (violation) {
          switch (policy.enforcement) {
            ...
            case 'warn':
              ...
              break;
            case 'monitor':
              await this.createAlert(secret.id, 'policy', 'medium', rule.message, { rule: rule.name });
              break;
          }
        }
      }
  ...
  async performHealthCheck(secretId: string): Promise<SecretHealthCheck> {
    ...
    const checkNames: Array<'expiration' | 'rotation' | 'usage' | 'compliance' | 'security'> = ['expiration', 'rotation', 'usage', 'compliance', 'security'];
    ...
    const checks: HealthCheck[] = results.map((result, index) => {
      if (result.status === 'fulfilled') {
        return result.value as HealthCheck;
      } else {
        // Return a failed check result for errors
        return {
          name: checkNames[index],
          type: checkNames[index],
    ...
    const overallStatus: 'healthy' | 'warning' | 'critical' | 'unknown' =
      checks.some(c => c.status === 'fail') ? 'critical' :
  ...
  async getSecretMetrics(secretId: string, period: string = '24h'): Promise<SecretMetrics> {
    ...
    const accessCount = secret.audit.accessCount;
    const failureCount = secret.audit.accessHistory.filter(a => !a.success).length;
    const averageAccessTime = 50; // Placeholder
    const uniqueUsers = new Set(secret.audit.accessHistory.map(a => a.userId)).size;
  ...
  private async checkSecretAccess(secret: Secret, userId: string, _action: string): Promise<boolean> {
  ...
  private async auditSecretAccess(secretId: string, action: string, success: boolean, error?: string): Promise<void> {
    // Log audit event
  ...
  private async trackSecretUsage(secretId: string, userId: string, action: string): Promise<void> {
  ...
  private async createAlert(secretId: string, type: 'expiration' | 'rotation' | 'access' | 'policy' | 'security', severity: 'low' | 'medium' | 'high' | 'critical', message: string, details: unknown): Promise<void> {
  ...
  private async validateEngineConfig(engine: SecretEngine): Promise<void> {
    // Validate engine configuration
  ...
  private async testEngineConnection(engine: SecretEngine): Promise<void> {
    // Test connection to engine
    logger.info(`Testing connection to ${engine.name}`);
  }

  private async syncWithAWS(engine: SecretEngine, secret: Secret): Promise<boolean> {
    // AWS Secrets Manager integration
    logger.info(`Syncing with AWS Secrets Manager: ${secret.name}`);
    return true;
  }

  private async syncWithAzure(engine: SecretEngine, secret: Secret): Promise<boolean> {
    // Azure Key Vault integration
    logger.info(`Syncing with Azure Key Vault: ${secret.name}`);
    return true;
  }

  private async syncWithVault(engine: SecretEngine, secret: Secret): Promise<boolean> {
    // HashiCorp Vault integration
    logger.info(`Syncing with HashiCorp Vault: ${secret.name}`);
    return true;
  }

  private async syncWithGCP(engine: SecretEngine, secret: Secret): Promise<boolean> {
  ...
  private generateOAuthToken(): string {
    const randomStr = Math.random().toString(36).substring(2, 15) + Math.random().toString(36).substring(2, 15) + Math.random().toString(36).substring(2, 15);
  ...
  private addRotationLog(job: SecretRotationJob, level: 'debug' | 'info' | 'warn' | 'error', message: string): void {
  ...
  private async rollbackRotation(job: SecretRotationJob): Promise<void> {
    // Rollback rotation
    logger.info(`Rolling back rotation for job ${job.id}`);
  }

  private async sendRotationNotifications(job: SecretRotationJob): Promise<void> {
    // Send notifications
  ...
  private async cancelRotationJobs(secretId: string): Promise<void> {
    const rotationJobs = Array.from(this.rotationJobs.values());
  ...
  private isPolicyApplicable(policy: SecretPolicy, secret: Secret): boolean {
  ...
  private async evaluatePolicyRule(_rule: unknown, _secret: Secret): Promise<boolean> {
    // Evaluate policy rule against secret
    return false; // No violation
  }

  private async checkSecretExpiration(secret: Secret): Promise<HealthCheck> {
    const expiration = secret.metadata.expiresAt;
    if (!expiration) {
      return {
        name: 'expiration',
        type: 'expiration',
    ...
    const daysUntilExpiration = Math.floor((new Date(expiration).getTime() - Date.now()) / (24 * 60 * 60 * 1000));
  ...
  private async checkRotationStatus(secret: Secret): Promise<HealthCheck> {
    ...
    const lastRotated = secret.metadata.lastRotated;
    if (!lastRotated) {
      return {
        name: 'rotation',
        type: 'rotation',
    ...
    const daysSinceRotation = Math.floor((Date.now() - new Date(lastRotated).getTime()) / (24 * 60 * 60 * 1000));
    const rotationInterval = secret.metadata.rotationInterval || 90;
  ...
  private async checkUsagePattern(secret: Secret): Promise<HealthCheck> {
    const recentUsage = this.usage.filter(u => u.secretId === secret.id &&
  ...
  private async checkCompliance(_secret: Secret): Promise<HealthCheck> {
  ...
  private async checkSecurityRisks(_secret: Secret): Promise<HealthCheck> {
        ...
        for (const secret of secrets) {
          if (secret.metadata.autoRotate && secret.metadata.rotationInterval) {
            ...
            if (lastRotated) {
              ...
              const daysSinceRotation = Math.floor((Date.now() - lastRotatedDate.getTime()) / (24 * 60 * 60 * 1000));

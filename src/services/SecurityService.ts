...
export class SecurityService {
  ...
  async enableMFA(userId: string, secret: string, verificationCode: string): Promise<boolean> {
    ...
    const isValid = await this.verifyMFACode(secret, verificationCode);
    if (!isValid) {
  ...
  async validateSession(token: string): Promise<{ valid: boolean; user?: User }> {
    const session = this.sessions.get(token) as { userId: string; expiresAt: Date } | undefined;
  ...
  async invalidateSession(token: string): Promise<void> {
    const session = this.sessions.get(token) as { userId: string } | undefined;
    if (session) {
  ...
  private cleanupExpiredSessions(): void {
    const now = new Date();
    for (const [token, session] of this.sessions.entries()) {
      if (now > (session as { expiresAt: Date }).expiresAt) {
        this.sessions.delete(token);
      }
    }
  }
  ...
  private detectSuspiciousLogin(events: SecurityAuditEvent[]): void {
    const loginEvents = events.filter(e => e.action.includes('login'));
    const loginsByIP = new Map<string, number>();
    loginEvents.forEach(event => {
      const count = loginsByIP.get(event.ipAddress) || 0;
  ...
  private detectPrivilegeEscalation(events: SecurityAuditEvent[]): void {
    const roleChanges = events.filter(e => e.action.includes('role') || e.action.includes('permission'));
  ...
  private async verifyMFACode(secret: string, code: string): Promise<boolean> {
    ...
    try {
      ...
      const expectedCode = CryptoJS.HmacSHA1(timeSlice.toString(), secret).toString().substring(0, 6);
  ...
  private assessRiskLevel(action: string, details: Record<string, unknown>): SecurityAuditEvent['riskLevel'] {
    const criticalActions = ['user:delete', 'system:restore', 'data:decrypt'];
    const highRiskActions = ['user:assign_roles', 'security:configure', 'system:backup'];

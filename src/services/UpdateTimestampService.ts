    ...
    try {
      ...
      if (saved) {
        ...
        Object.entries(data).forEach(([key, value]: [string, any]) => {
          if (value && typeof value === 'object' && 'component' in value && 'updateType' in value) {
            this.timestamps.set(key, {
              component: value.component as string,
              timestamp: new Date(value.timestamp as string),
              updateType: value.updateType as string,
...
export function useUpdateTimestamp(component?: string): { workflowLastUpdate: string } {
  ...
  React.useEffect(() => {
    ...
    const unsubscribe = updateTimestampService.subscribe(componentKey, () => {
    ...
    const refreshTimestamp = () => {
      ...
      timerRef.current = setTimeout(refreshTimestamp, nextUpdate);
    };

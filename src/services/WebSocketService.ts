...
export class WebSocketService extends BaseService {
  ...
  public async connect(): Promise<void> {
    ...
    try {
      await this.executeOperation('connect', async () => {
        ...
        this.ws = new WebSocket(url, this.wsConfig.protocols);
        this.ws.binaryType = this.wsConfig.binaryType;
      ...
      if (this.wsConfig.reconnect && this.reconnectAttempts < this.wsConfig.maxReconnectAttempts) {
  ...
  public subscribe<T = unknown>(
    ...
    this.subscriptions.get(event)!.add(callback as (data: Record<string, unknown>) => void);
    ...
    return () => {
      this.unsubscribe(event, callback as (data: Record<string, unknown>) => void);
    };
  }
  ...
  private handleOpen(_event: Event): void {
    ...
    if (this.wsConfig.authentication) {
  ...
  private handleEvent(message: WebSocketMessage): void {
    const event = message.data as WebSocketEvent;
  ...
  private handleErrorMessage(message: WebSocketMessage): void {
    const errorData = message.data as { message?: string; code?: string; details?: unknown };
    const error = new Error(errorData.message || 'WebSocket error');
    (error as unknown as Record<string, unknown>).code = errorData.code;
    (error as unknown as Record<string, unknown>).details = errorData.details;
  ...
  private async sendMessage(message: WebSocketMessage): Promise<void> {
    ...
    try {
      const data = this.wsConfig.binaryType === 'arraybuffer'
  ...
  private startHeartbeat(): void {
    ...
    this.pingInterval = setInterval(() => {
      if (this.connectionState === 'connected' && this.ws && this.ws.readyState === WebSocket.OPEN) {
        ...
        setTimeout(() => {
          if (Date.now() - this.lastPingTime > this.wsConfig.pongTimeout) {
  ...
  private scheduleReconnect(): void {
    ...
    const delay = this.wsConfig.reconnectDelay * Math.pow(2, this.reconnectAttempts - 1);
  ...
  private buildConnectionUrl(): string {
    ...
    if (this.wsConfig.authentication) {
      const { type, token, apiKey } = this.wsConfig.authentication;
  ...
  private async authenticate(): Promise<void> {
    if (!this.wsConfig.authentication) return;

    const { type, token, apiKey, custom } = this.wsConfig.authentication;

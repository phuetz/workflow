...
/**
 * Minimal interface for a node in a snapshot
 */
...
/**
 * Minimal interface for an edge in a snapshot
 */
...
export class WorkflowDiffService {
  ...
  private compareNodes(oldNodes: unknown[], newNodes: unknown[]): NodeDiff[] {
    ...
    const oldNodesMap = new Map(
      (oldNodes as SnapshotNode[]).map((n) => [n.id, n])
    );
    const newNodesMap = new Map(
      (newNodes as SnapshotNode[]).map((n) => [n.id, n])
    );
  ...
  private compareEdges(oldEdges: unknown[], newEdges: unknown[]): EdgeDiff[] {
    ...
    const oldEdgesMap = new Map(
      (oldEdges as SnapshotEdge[]).map((e) => [e.id, e])
    );
    const newEdgesMap = new Map(
      (newEdges as SnapshotEdge[]).map((e) => [e.id, e])
    );
  ...
  private detectPropertyChanges(
    ...
    for (const key of allKeys) {
      ...
      if (typeof oldValue === 'object' && typeof newValue === 'object') {
        changes.push(
          ...this.detectPropertyChanges(oldValue as Record<string, unknown>, newValue as Record<string, unknown>, [...path, key])
        );
  ...
  private detectConflicts(diff: WorkflowDiff): Conflict[] {
    ...
    for (const edgeDiff of diff.edges) {
      if (edgeDiff.type !== 'removed') {
        const edge = edgeDiff.newEdge as SnapshotEdge | undefined;
        if (
          edge &&
          ((edge.source && removedNodeIds.has(edge.source)) || (edge.target && removedNodeIds.has(edge.target)))
        ) {
          conflicts.push({
            type: 'edge',
  ...
  private mergeNodes(
    ...
    const baseNodesTyped = baseNodes as SnapshotNode[];
    const nodes1Typed = nodes1 as SnapshotNode[];
    const nodes2Typed = nodes2 as SnapshotNode[];

    // Process nodes from both versions
    for (const node of [...nodes1Typed, ...nodes2Typed]) {
      const nodeId = node.id;

      if (processedIds.has(nodeId)) continue;

      const baseNode = baseNodesTyped.find((n) => n.id === nodeId);
      const node1 = nodes1Typed.find((n) => n.id === nodeId);
      const node2 = nodes2Typed.find((n) => n.id === nodeId);
  ...
  private mergeEdges(
    ...
    const edges1Typed = edges1 as SnapshotEdge[];
    const edges2Typed = edges2 as SnapshotEdge[];

    for (const edge of [...edges1Typed, ...edges2Typed]) {
      const edgeId = edge.id;

      if (processedIds.has(edgeId)) continue;

      const edge1 = edges1Typed.find((e) => e.id === edgeId);
      const edge2 = edges2Typed.find((e) => e.id === edgeId);

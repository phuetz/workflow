...
export class WorkflowImportService {
  ...
  private validateWorkflowData(data: Record<string, unknown>): ImportValidationResult {
    ...
    return {
      ...
      summary: {
        ...
        edgeCount,
        supportedNodes,
        unsupportedNodes: Array.from(new Set(unsupportedNodes))
      }
    };
  }
  ...
  private processWorkflowData(data: Record<string, unknown>): ImportedWorkflow {
    ...
    const processedNodes: WorkflowNode[] = (data.nodes as Record<string, unknown>[]).map((node: Record<string, unknown>) => {
      ...
      const nodePosition = node.position as { x: number; y: number } | undefined;
      const position = nodePosition || { x: 0, y: 0 };
      const nodeType = (nodeData?.type as string) || 'custom';
      const nodeTypeInfo = nodeTypes[nodeType];

      return {
        id: newId,
        type: node.type as string || 'custom',
        position: position,
        data: {
          id: newId,
          type: nodeType,
          label: (nodeData?.label as string) || nodeTypeInfo?.label || 'Unknown Node',
          position: position,
          icon: (nodeData?.icon as string) || nodeTypeInfo?.icon || '⚙️',
          color: (nodeData?.color as string) || nodeTypeInfo?.color || '#999999',
          inputs: (nodeData?.inputs as number) || nodeTypeInfo?.inputs || 1,
          outputs: (nodeData?.outputs as number) || nodeTypeInfo?.outputs || 1,
          config: (nodeData?.config as Record<string, unknown>) || {},
          pinnedData: nodeData?.pinnedData as NodeData['pinnedData']
        },
        selected: false,
        dragging: false,
      } as WorkflowNode;
    ...
    const processedEdges: WorkflowEdge[] = (data.edges as Record<string, unknown>[]).map((edge: Record<string, unknown>) => ({
      id: generateNewId(),
      source: (nodeIdMap.get(edge.source as string) || edge.source) as string,
      target: (nodeIdMap.get(edge.target as string) || edge.target) as string,
      ...
      labelShowBg: edge.labelShowBg as boolean | undefined,
      labelBgStyle: edge.labelBgStyle,
      animated: (edge.animated as boolean) || false,
      hidden: edge.hidden as boolean | undefined,
      data: edge.data,
    } as WorkflowEdge));
    ...
    const workflow: ImportedWorkflow = {
      id: (data.id as string) || generateNewId(),
      ...
      metadata: {
        ...
        category: (metadata?.category as string) || 'imported',
      },
      settings: {
        variables: (settings?.variables as Record<string, unknown>) || {},
        environment: (settings?.environment as string) || 'development',
        schedule: settings?.schedule as Record<string, unknown> | undefined,
      }
    };

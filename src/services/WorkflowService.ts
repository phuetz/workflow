...
export class WorkflowService extends BaseService {
  ...
  private loadWorkflowsFromStore(): void {
    try {
      ...
      const { nodes, edges, workflows } = store;
      ...
      Object.entries(workflows).forEach(([id, workflowData]) => {
        const workflow: Workflow = {
          id,
          name: (workflowData as any).name || `Workflow ${id}`,
          description: (workflowData as any).description,
          nodes: (workflowData as any).nodes || [],
          edges: (workflowData as any).edges || [],
          createdAt: new Date((workflowData as any).createdAt || Date.now()),
          updatedAt: new Date((workflowData as any).updatedAt || Date.now()),
          lastExecutedAt: (workflowData as any).lastExecutedAt ? new Date((workflowData as any).lastExecutedAt) : undefined,
          executionCount: (workflowData as any).executionCount || 0,
          isActive: (workflowData as any).isActive !== false,
          version: (workflowData as any).version || 1,
          tags: (workflowData as any).tags,
          metadata: (workflowData as any).metadata
        };
  ...
  async importWorkflows(data: Record<string, unknown>): Promise<void> {
    Object.entries(data).forEach(([id, workflowData]) => {
      const workflow: Workflow = {
        ...(workflowData as any),
        createdAt: new Date((workflowData as any).createdAt),
        updatedAt: new Date((workflowData as any).updatedAt),
        lastExecutedAt: (workflowData as any).lastExecutedAt ? new Date((workflowData as any).lastExecutedAt) : undefined
      };
      this.workflows.set(id, workflow);
    });

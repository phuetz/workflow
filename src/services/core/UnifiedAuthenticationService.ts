...
export class UnifiedAuthenticationService {
  ...
  async authenticate(email: string, password: string, ipAddress?: string): Promise<AuthResult> {
    return withErrorHandling(
      async () => {
        ...
        // Emit login event
        eventNotificationService.emitEvent('auth.login', { userId: user.id }, 'UnifiedAuthenticationService');
  ...
  async validateToken(token: string): Promise<{ valid: boolean; user?: User }> {
    try {
      ...
      await cacheService.set(`token:${token}`, result, 300);
  ...
  async logout(token: string): Promise<void> {
    try {
      ...
      if (session) {
        ...
        eventNotificationService.emitEvent('auth.logout', { userId: session.userId }, 'UnifiedAuthenticationService');
  ...
  private async updateUser(user: User): Promise<void> {
    ...
    // Update cache
    await cacheService.set(`user:id:${user.id}`, user, 3600);
    await cacheService.set(`user:email:${user.email}`, user, 3600);
  }

...
export class WorkflowOrchestrationService {
  ...
  private async executeWorkflowSync(
    ...
    try {
      ...
      for (const group of plan.parallelGroups) {
        ...
        if (['cancelled', 'failed'].includes(context.status)) {
          break;
        }
      }
  ...
  private async executeNode(
    ...
    try {
      ...
      eventNotificationService.emit('node.execution.start', {
      ...
      eventNotificationService.emit('node.execution.complete', {
        executionId: context.executionId,
        nodeId: step.nodeId,
  ...
  private saveCheckpoints(): void {
    const executionContexts = Array.from(this.executions.values());
  ...
  private calculateNodePriority(node: Node, dependencies: Map<string, string[]>): number {
    ...
    for (const [nodeId, deps] of dependencyEntries) {

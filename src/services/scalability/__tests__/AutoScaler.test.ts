...
describe('IntelligentAutoScaler', () => {
  ...
  describe('Reactive Scaling', () => {
    beforeEach(async () => {
      autoScaler = new IntelligentAutoScaler({
        ...
        cooldownPeriod: 5000,
        warmupTime: 100,  // Short warmup for tests
        scalingPolicy: {
          type: 'reactive',
          rules: []
        }
      ...
      await autoScaler.start();
      await vi.advanceTimersByTimeAsync(150);  // Allow instances to become 'running'
    });
    ...
    it('should scale up on high CPU usage', async () => {
      ...
      // Simulate high CPU - mark as test metrics to prevent override
      const instances = autoScaler.getInstances();
      instances.forEach(instance => {
        instance.metrics.cpuUsage = 85;
        instance.metrics.memoryUsage = 85;
        (instance as Record<string, unknown>)._testMetrics = true;
      });
    ...
    it('should scale down on low usage', async () => {
      ...
      // Simulate low usage - mark as test metrics
      const instances = autoScaler.getInstances();
      instances.forEach(instance => {
        instance.metrics.cpuUsage = 15;
        instance.metrics.memoryUsage = 15;
        (instance as Record<string, unknown>)._testMetrics = true;
      });
    ...
    it('should respect cooldown period', async () => {
      ...
      instances.forEach(instance => {
        ...
        instance.metrics.memoryUsage = 90;
      });
  ...
  describe('Event Emissions', () => {
    ...
    it('should emit health issue events', async () => {
      const eventPromise = new Promise<void>((resolve) => {

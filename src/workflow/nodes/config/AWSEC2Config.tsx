/**
 * AWS EC2 Node Configuration
 */

import React, { useState } from 'react';

interface AWSEC2ConfigProps {
  config: Record<string, unknown>;
  onChange: (config: Record<string, unknown>) => void;
}

export const AWSEC2: React.FC<AWSEC2ConfigProps> = ({ config, onChange }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-orange-50 border border-orange-200 rounded-md p-3">
        ...
        <p className="text-xs text-orange-700">Requires AWS EC2 API credentials.</p>
      </div>
    </div>
  );
};

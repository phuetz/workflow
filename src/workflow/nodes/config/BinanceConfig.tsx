...
interface BinanceConfigProps {
  config: Record<string, unknown>;
  onChange: (config: Record<string, unknown>) => void;
}

export const Binance: React.FC<BinanceConfigProps> = ({ config, onChange }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-yellow-50 border border-yellow-200 rounded-md p-3">
        ...
        <p className="text-xs text-yellow-700">Requires Binance API credentials.</p>
      </div>
    </div>
  );
};

...
interface CSVParserV2ConfigProps {
  config: Record<string, unknown>;
  onChange: (config: Record<string, unknown>) => void;
}

export const CSVParserV2: React.FC<CSVParserV2ConfigProps> = ({ config, onChange }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-teal-50 border border-teal-200 rounded-md p-3">
        <p className="text-sm text-teal-900 font-medium mb-1">Configuration</p>
        <p className="text-xs text-teal-700">CSV Parser utility for data processing.</p>
      </div>
    </div>
  );
};

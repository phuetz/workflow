/**
 * Cloudflare Workers Node Configuration
 */

import React, { useState } from 'react';

interface CloudflareWorkersConfigProps {
  config: Record<string, unknown>;
  onChange: (config: Record<string, unknown>) => void;
}

export const CloudflareWorkers: React.FC<CloudflareWorkersConfigProps> = ({ config, onChange }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-orange-50 border border-orange-200 rounded-md p-3">
        ...
        <p className="text-xs text-orange-700">Requires Cloudflare Workers API credentials.</p>
      </div>
    </div>
  );
};

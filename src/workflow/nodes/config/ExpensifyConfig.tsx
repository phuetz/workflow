/**
 * Expensify Node Configuration
 */

import React, { useState } from 'react';

interface ExpensifyConfigProps {
  config: Record<string, unknown>;
  onChange: (config: Record<string, unknown>) => void;
}

export const Expensify: React.FC<ExpensifyConfigProps> = ({ config, onChange }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-green-50 border border-green-200 rounded-md p-3">
        ...
        <p className="text-xs text-green-700">Requires Expensify API credentials.</p>
      </div>
    </div>
  );
};

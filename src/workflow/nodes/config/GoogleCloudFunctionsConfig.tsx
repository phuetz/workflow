/**
 * Google Cloud Functions Node Configuration
 */

import React, { useState } from 'react';

interface GoogleCloudFunctionsConfigProps {
  config: Record<string, unknown>;
  onChange: (config: Record<string, unknown>) => void;
}

export const GoogleCloudFunctions: React.FC<GoogleCloudFunctionsConfigProps> = ({ config, onChange }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-md p-3">
        ...
        <p className="text-xs text-blue-700">Requires Google Cloud Functions API credentials.</p>
      </div>
    </div>
  );
};

/**
 * Google Cloud Run Node Configuration
 */

import React, { useState } from 'react';

interface GoogleCloudRunConfigProps {
  config: Record<string, unknown>;
  onChange: (config: Record<string, unknown>) => void;
}

export const GoogleCloudRun: React.FC<GoogleCloudRunConfigProps> = ({ config, onChange }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-md p-3">
        ...
        <p className="text-xs text-blue-700">Requires Google Cloud Run API credentials.</p>
      </div>
    </div>
  );
};

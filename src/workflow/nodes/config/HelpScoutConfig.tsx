/**
 * Help Scout Node Configuration
 */

import React, { useState } from 'react';

interface HelpScoutConfigProps {
  config: Record<string, unknown>;
  onChange: (config: Record<string, unknown>) => void;
}

export const HelpScout: React.FC<HelpScoutConfigProps> = ({ config, onChange }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-indigo-50 border border-indigo-200 rounded-md p-3">
        ...
        <p className="text-xs text-indigo-700">Requires Help Scout API credentials.</p>
      </div>
    </div>
  );
};

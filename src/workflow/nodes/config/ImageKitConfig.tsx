/**
 * ImageKit Node Configuration
 */

import React, { useState } from 'react';

interface ImageKitConfigProps {
  config: Record<string, unknown>;
  onChange: (config: Record<string, unknown>) => void;
}

export const ImageKit: React.FC<ImageKitConfigProps> = ({ config, onChange }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-purple-50 border border-purple-200 rounded-md p-3">
        ...
        <p className="text-xs text-purple-700">Requires ImageKit API credentials.</p>
      </div>
    </div>
  );
};

/**
 * Kustomer Node Configuration
 */

import React, { useState } from 'react';

interface KustomerConfigProps {
  config: Record<string, unknown>;
  onChange: (config: Record<string, unknown>) => void;
}

export const Kustomer: React.FC<KustomerConfigProps> = ({ config, onChange }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-pink-50 border border-pink-200 rounded-md p-3">
        ...
        <p className="text-xs text-pink-700">Requires Kustomer API credentials.</p>
      </div>
    </div>
  );
};

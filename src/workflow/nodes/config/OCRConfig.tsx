...
interface OCRConfigProps {
  config: Record<string, unknown>;
  onChange: (config: Record<string, unknown>) => void;
}

export const OCR: React.FC<OCRConfigProps> = ({ config, onChange }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-violet-50 border border-violet-200 rounded-md p-3">
        <p className="text-sm text-violet-900 font-medium mb-1">Configuration</p>
        <p className="text-xs text-violet-700">OCR utility for data processing.</p>
      </div>
    </div>
  );
};

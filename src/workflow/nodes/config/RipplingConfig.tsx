/**
 * Rippling Node Configuration
 */

import React, { useState } from 'react';

interface RipplingConfigProps {
  config: Record<string, unknown>;
  onChange: (config: Record<string, unknown>) => void;
}

export const Rippling: React.FC<RipplingConfigProps> = ({ config, onChange }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-violet-50 border border-violet-200 rounded-md p-3">
        ...
        <p className="text-xs text-violet-700">Requires Rippling API credentials.</p>
      </div>
    </div>
  );
};

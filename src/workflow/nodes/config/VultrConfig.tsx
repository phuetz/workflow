/**
 * Vultr Node Configuration
 */

import React, { useState } from 'react';

interface VultrConfigProps {
  config: Record<string, unknown>;
  onChange: (config: Record<string, unknown>) => void;
}

export const Vultr: React.FC<VultrConfigProps> = ({ config, onChange }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-md p-3">
        ...
        <p className="text-xs text-blue-700">Requires Vultr API credentials.</p>
      </div>
    </div>
  );
};

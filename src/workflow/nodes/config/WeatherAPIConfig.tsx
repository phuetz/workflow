/**
 * WeatherAPI Node Configuration
 */

import React, { useState } from 'react';

interface WeatherAPIConfigProps {
  config: Record<string, unknown>;
  onChange: (config: Record<string, unknown>) => void;
}

export const WeatherAPI: React.FC<WeatherAPIConfigProps> = ({ config, onChange }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-cyan-50 border border-cyan-200 rounded-md p-3">
        <p className="text-sm text-cyan-900 font-medium mb-1">Configuration</p>
        <p className="text-xs text-cyan-700">WeatherAPI utility for data processing.</p>
      </div>
    </div>
  );
};
